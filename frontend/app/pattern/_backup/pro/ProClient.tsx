  ...
  useEffect(() => {
    ...
    async function loadWeatherFromLocation(
      ...
      try {
        const resp = await fetch(
          `/api/current-conditions?lat=${latitude}&lon=${longitude}`
        );

        if (!resp.ok) {
          const text = await resp.text();
          console.log("Weather fetch failed:", text);
          setAutoError("Could not auto-detect weather; fill manually.");
          return;
        }
        ...
        if (typeof data.temp_f === "number") {
          setTempF(String(Math.round(data.temp_f)));
        }
        if (typeof data.wind_speed === "number") {
          setWindSpeed(String(Math.round(data.wind_speed)));
        }
        if (data.sky_condition) {
          setSkyCondition(data.sky_condition);
        }
      } catch (err) {
        console.log("Weather exception:", err);
        setAutoError("Failed to auto-load weather; fill manually.");
      } finally {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        loadWeatherFromLocation(latitude, longitude);
      },
      (err) => {
